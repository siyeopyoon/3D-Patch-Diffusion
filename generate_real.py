...
rootpath = "/external"
itersname="000610"
network_dir=f"{rootpath}/1_Codes/PatchDiffusion/network-snapshot-{itersname}.pkl"
out_dir = f"{rootpath}/1_Codes/PatchDiffusion/results/"
data_dir = f"{rootpath}/1_Codes/PatchDiffusion/realdata/"
...
@click.option('--steps', 'num_steps',      help='Number of sampling steps', metavar='INT',                          type=click.IntRange(min=1), default=500, show_default=True)
...
def main(network_pkl, resolution, on_latents, embed_fq, mask_pos, x_start, y_start, image_size, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.RealDataset', path=data_dir,
    ...
    for batch_seeds in range(len(dataset_iterator)):
        ...
        for _ in range(batch_size):
            filename_, images_ = next(dataset_iterator)
            images.append(images_), filename.append(filename_)
        ...
        resolution=512
        #x_start,y_start =0,0
        #images=images[:,:,y_start:y_start+resolution,x_start:x_start+resolution,:]
        #volumes = volumes[:, :, y_start:y_start + resolution, x_start:x_start + resolution, :]
        dist.print0(images.shape)
        ...
        latents_pos = latents_pos.unsqueeze(0).unsqueeze(-1).repeat(batch_size,  1,1, 1,d)


        # Generate images.
        sampler_kwargs = {key: value for key, value in sampler_kwargs.items() if value is not None}

        for randIDX in range(10):
            torch.manual_seed(randIDX)
            latents = torch.randn([batch_size, image_channel, image_size, image_size, d], device=device)

            Aimages = edm_sampler(net, latents, images, latents_pos, mask_pos=False, class_labels=None, randn_like=torch.randn_like, **sampler_kwargs)
            Aimages=torch.squeeze(Aimages)

            # Save images.
            images_np =Aimages.cpu().numpy()


            np.save(f'{out_dir}_REAL_network{itersname}_{batch_seeds}_generated_{randIDX}.npy',images_np)
